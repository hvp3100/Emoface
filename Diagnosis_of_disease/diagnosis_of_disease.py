...
from torchvision.transforms import functional as L
...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default= '/input/datasets3class/datasetsthree/raf_basic', help='Raf-DB dataset path.')
    ...
    parser.add_argument('--margin_1', type=float, default=0.05, help='Rank regularization margin. Details described in the paper.')
...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, transform = None, basic_aug = False):
        ...
        NAME_COLUMN = 0
        LABEL_COLUMN = 1
        df = pd.read_csv(os.path.join(self.raf_path, 'EmoLabel/list_label.txt'), sep=' ', header=None)
        if phase == 'train':
            dataset = df[df[NAME_COLUMN].str.startswith('train')]
        else:
            dataset = df[df[NAME_COLUMN].str.startswith('test')]
        file_names = dataset.iloc[:, NAME_COLUMN].values
        self.label = dataset.iloc[:, LABEL_COLUMN].values - 1 # 0:MDD, 1:BPD, 2:HCs
...
class RandomFiveCrop(object):
    ...
    def __call__(self, img):
        ...
        return L.five_crop(img, self.size)[random.randint(0, 4)]
...
class Res18Feature(nn.Module):
    def __init__(self, pretrained = True, num_classes = 3, drop_rate = 0.5):
...
#                 label_idx = indexes[update_idx]  # get samples' index in train_loader
                label_idx = indexes.to(update_idx.device)
                relabels = predicted_labels.to(update_idx.device)
