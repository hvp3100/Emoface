...
class DecaModule(LightningModule):
    ...
    def reconfigure(self, model_params, inout_params, learning_params, stage_name="", downgrade_ok=False, train=True):
        ...
        if (self.mode == DecaMode.DETAIL and model_params.mode != DecaMode.DETAIL) and not downgrade_ok:
            raise RuntimeError("You're switching the mode from DETAIL to COARSE. Is this really what you want?!")
        self.inout_params = inout_params
        self.learning_params = learning_params
        ...
        print(f"MODE RECONFIGURED TO: {self.mode}")
    ...
    def encode(self, batch, training=True) -> dict:
        ...
        # valence / arousal - not necessary unless we want to use VA for supervision
        if 'va' in batch:
            va = batch['va']
        ...
        # 7 basic expression - not necessary unless we want to use expression for supervision
        if 'expr7' in batch:
            expr7 = batch['expr7']
        ...
        # affectnet basic expression - not necessary unless we want to use expression for supervision
...
class ExpDECA(DECA):
    ...
    def _create_model(self):
        ...
        if self.config.expression_backbone == 'deca_parallel':
            ...
            ## b) Clones the original DECA coarse decoder (and the entire decoder will be trainable)
...
def instantiate_deca(cfg, stage, prefix, checkpoint=None, checkpoint_kwargs=None):
    ...
    if checkpoint is None:
        ...
        if cfg.model.resume_training:
            ...
            print("[WARNING] Loading checkpoint pretrained by the old code")
