...
def save_obj(emoface, filename, opdict, i=0):
    dense_template_path = Path(gdl.__file__).parents[1] / 'assets' / "EMOFACE" / "data" / 'texture_data_256.npy'
    ...
    faces = emoface.deca.render.faces[0].detach().cpu().numpy()
    texture = util.tensor2image(opdict['uv_texture_gt'][i])
    uvcoords = emoface.deca.render.raw_uvcoords[0].detach().cpu().numpy()
    uvfaces = emoface.deca.render.uvfaces[0].detach().cpu().numpy()
...
def decode(emoface, values, training=False):
    with torch.no_grad():
        values = emoface.decode(values, training=training)
        uv_detail_normals = None
        if 'uv_detail_normals' in values.keys():
            uv_detail_normals = values['uv_detail_normals']
        visualizations, grid_image = emoface._visualization_checkpoint(
            values['verts'],
            values['trans_verts'],
            values['ops'],
            uv_detail_normals,
